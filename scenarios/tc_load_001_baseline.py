...
# 🎯 Обновлено: 2024-12-16 на основе 8 baseline запусков
...
# 🎯 Обновлено: 2024-12-16 на основе 8 baseline запусков
# 📊 Baseline P95: 1.3s (но нестабильно: 0.6s-8.4s range)
# ⚠️ ВАЖНО: Dashboard load очень нестабилен, используем мягкий порог
# ✏️ Как изменить: замените на реальное значение P95 * 1.5-2.0 (из-за нестабильности)
_metrics_collector.define_slo(
    name="dashboard_duration",      # Имя метрики (НЕ МЕНЯТЬ!)
    threshold=3.0,                   # P95 (2.5s безопасное) × 1.2 = 3.0s
    comparison="less_than"           # Должно быть МЕНЬШЕ порога
)
