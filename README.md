...
│   └── report_engine.py        # Unified reporting system
...
# Baseline метрики для сравнения (TC-LOAD-002+)
baseline_metrics:
  "500mb":
    file_size_mb: 500
    csv_upload: 24.9        # ← Обновите из отчета TC-LOAD-001
    dag1_duration: 31.8     # ← P95 из отчета TC-LOAD-001
    dag2_duration: 46.1     # ← P95 из отчета TC-LOAD-001
    dashboard_load: 0.5     # ← P95 из отчета TC-LOAD-001
```

**Как настроить baseline:**
1. Запустите TC-LOAD-001 и получите отчет
2. Найдите P95 значения для каждой метрики
3. Обновите значения в `baseline_metrics` секции
4. TC-LOAD-002 автоматически загрузит их для сравнения
...
После завершения тестов автоматически генерируются отчеты в **3 форматах**:

**Для TC-LOAD-001:**
```
./logs/tc_load_001_report_20241205_143022.txt   # Текстовый отчет
./logs/tc_load_001_report_20241205_143022.json  # JSON для автоматизации
./logs/tc_load_001_runs_20241205_143022.csv     # CSV для Excel анализа
```
...
**SLO (Service Level Objective)** - жесткие пороги Pass/Fail для автоматизации:
- Устанавливаются в коде теста (например, `dag1_duration < 450s`)
- Используются для автоматической проверки в CI/CD
- Если SLO FAIL → тест падает, релиз блокируется
- Пример: `SLO dag1_duration (< 450s): PASS ✓`

**Baseline** - справочные значения из TC-LOAD-001 для анализа трендов:
...
- **Baseline** отвечает на вопрос: "Насколько хорошо/плохо работает?"
